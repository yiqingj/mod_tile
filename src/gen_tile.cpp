...
struct projectionconfig * get_projection(const char * srs) {
	...
	if (strstr(srs, "+proj=merc +a=6378137 +b=6378137") != NULL) {
		...
		prj = (struct projectionconfig *) malloc(
				sizeof(struct projectionconfig));
...
static void parameterize_map_max_connections(Map &m, int num_threads) {
	...
	for (i = 0; i < m.layer_count(); i++) {
		...
		std::shared_ptr<datasource> ds = datasource_cache::instance().create(
				params);
#else
...
#ifdef METATILE
mapnik::box2d<double> tile2prjbounds(struct projectionconfig * prj, int x,
		int y, int z) {

	int render_size_tx = MIN(METATILE, prj->aspect_x * (1 << z));
	int render_size_ty = MIN(METATILE, prj->aspect_y * (1 << z));

	double p0x = prj->bound_x0
			+ (prj->bound_x1 - prj->bound_x0)
					* ((double) x / (double) (prj->aspect_x * 1 << z));
	double p0y = (prj->bound_y1
			- (prj->bound_y1 - prj->bound_y0)
					* (((double) y + render_size_ty)
							/ (double) (prj->aspect_y * 1 << z)));
	double p1x = prj->bound_x0
			+ (prj->bound_x1 - prj->bound_x0)
					* (((double) x + render_size_tx)
							/ (double) (prj->aspect_x * 1 << z));
	double p1y = (prj->bound_y1
			- (prj->bound_y1 - prj->bound_y0)
					* ((double) y / (double) (prj->aspect_y * 1 << z)));

	syslog(LOG_DEBUG,
			"Rendering projected coordinates %i %i %i -> %f|%f %f|%f to a %i x %i tile\n",
...
static enum protoCmd render(struct xmlmapconfig * map, int x, int y, int z,
	...
	syslog(LOG_DEBUG, "Render XMLNAME:: %s", map->xmlname);

	if (strncmp("vector", map->xmlname, strlen("vector"))) {
		syslog(LOG_DEBUG, "Rendering Image Tile");
		...
		for (yy = 0; yy < render_size_ty; yy++) {
			for (xx = 0; xx < render_size_tx; xx++) {
				mapnik::image_view<mapnik::image_data_32> vw(xx * map->tilesize,
						yy * map->tilesize, map->tilesize, map->tilesize,
						buf.data());
				tiles.set(xx, yy, save_to_string(vw, "png256"));
			}
		}
	} else { // generate vector tile!
		syslog(LOG_DEBUG, "Rendering Vector Tile");
		...
		for (yy = 0; yy < render_size_ty; yy++) {
			for (xx = 0; xx < render_size_tx; xx++) {
				...
				try {
					...
					syslog(LOG_ERR, "ERROR: failed to render TILE %s %d %d %d",
							map->xmlname, z, x + xx, y + yy);
					syslog(LOG_ERR, "   reason: %s", ex.what());
					return cmdNotDone;
				}
			}
...
void *render_thread(void * arg) {
	...
	for (iMaxConfigs = 0; iMaxConfigs < XMLCONFIGS_MAX; ++iMaxConfigs) {
		...
		maps[iMaxConfigs].store = init_storage_backend(
				parentxmlconfig[iMaxConfigs].tile_dir);
		...
		maps[iMaxConfigs].parameterize_function =
				init_parameterization_function(
		...
		if (maps[iMaxConfigs].store) {
			...
			try {
				mapnik::load_map(maps[iMaxConfigs].map,
						maps[iMaxConfigs].xmlfile);
				...
				if (parentxmlconfig[iMaxConfigs].num_threads > 10) {
					syslog(LOG_INFO,
							"Updating max_connection parameter for mapnik layers to reflect thread count");
					parameterize_map_max_connections(maps[iMaxConfigs].map,
							parentxmlconfig[iMaxConfigs].num_threads);
				}
				maps[iMaxConfigs].prj = get_projection(
						maps[iMaxConfigs].map.srs().c_str());
			} catch (std::exception const& ex) {
				syslog(LOG_ERR,
...
#ifdef HTCP_EXPIRE_CACHE
			strcpy(maps[iMaxConfigs].xmluri,
					parentxmlconfig[iMaxConfigs].xmluri);
			strcpy(maps[iMaxConfigs].host, parentxmlconfig[iMaxConfigs].host);
			strcpy(maps[iMaxConfigs].htcphost,
					parentxmlconfig[iMaxConfigs].htcpip);
			if (strlen(maps[iMaxConfigs].htcphost) > 0) {
				...
				if (maps[iMaxConfigs].htcpsock > 0) {
					syslog(LOG_INFO,
							"Successfully opened socket for HTCP cache expiry");
				} else {
					syslog(LOG_ERR,
							"Failed to opened socket for HTCP cache expiry");
				}
			} else {
				...
				if (!strcmp(maps[i].xmlname, req->xmlname)) {
					if (maps[i].ok) {
						if (check_xyz(item->mx, item->my, req->z, &(maps[i]))) {
							...
							metaTile tiles(req->xmlname, req->options, item->mx,
									item->my, req->z);
							...
							struct stat_info sinfo = maps[i].store->tile_stat(
									maps[i].store, req->xmlname, req->options,
									item->mx, item->my, req->z);

							if (sinfo.size > 0)
								syslog(LOG_DEBUG,
										"DEBUG: START TILE %s %d %d-%d %d-%d, age %.2f days",
										req->xmlname, req->z, item->mx,
										item->mx + size - 1, item->my,
										item->my + size - 1,
							...
							ret = render(&(maps[i]), item->mx, item->my, req->z,
									req->options, tiles);
							...
							if (ret == cmdDone) {
								try {
...
#ifdef HTCP_EXPIRE_CACHE
									tiles.expire_tiles(maps[i].htcpsock,
											maps[i].host, maps[i].xmluri);
#endif
									...
									syslog(LOG_ERR,
											"Failed writing metatile to disk with unknown error, requesting exit.");
									ret = cmdNotDone;
									request_exit();
								}
							}
							...
							syslog(LOG_WARNING,
									"Received request for map layer %s is outside of acceptable bounds z(%i), x(%i), y(%i)",
